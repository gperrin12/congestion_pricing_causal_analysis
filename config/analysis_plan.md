...
| 2026-07-20 | 7 | S6 first-stage primary series substituted: into-Manhattan B&T daily crossings (`panel_bt_day`, facilities Queens Midtown WB / Hugh L. Carey NB / Henry Hudson SB) because public CRZ vehicle entries (`t6yz-b64h`) begin at T0 with no pre-period. CRZ daily panel retained for descriptive reconciliation vs MTA 11-13% benchmark. Primary BSTS post window ends 2025-12-31 (before 2026-01-04 B&T toll hike). Lincoln/Holland not in B&T dataset (Port Authority). CausalImpact library post starts day after registered `pre_end` (includes washout for forecast continuity); reported ATT averages T0 onward only. | No | analysis_plan.md §7 contingency; config in `treatment.yaml` → `bsts` |
| 2026-08-10 | 6, 9 | Holiday / Christmas-New Year trough adjustment: primary S1/S2 drop federal-holiday weeks and Dec 24-Jan 2 trough weeks; primary S6 BSTS adds `holiday` + `winter_trough` covariates and averages ATT on non-holiday/non-trough post days only (series stays continuous for CausalImpact). Robustness retains treated x holiday/trough interactions (DiD) and full-window ATT (BSTS). Config in `treatment.yaml` → `holiday_adjustment`. | Yes | Differential holiday trough concern for treated units; T0 (2025-01-05) sits in the quiet window |
